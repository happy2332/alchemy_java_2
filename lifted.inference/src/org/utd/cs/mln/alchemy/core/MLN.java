...
public class MLN {
	...
	public static WClause create_new_clause(WClause clause) {
		...
		new_clause.weight = new LogDouble(clause.weight);
		new_clause.satHyperCubes = new ArrayList<Boolean>(clause.satHyperCubes);
		new_clause.hcNumCopies = new ArrayList<Integer>(clause.hcNumCopies);
		/*
		for(HyperCube hyperCube : clause.hyperCubes)
			new_clause.hyperCubes.add(new HyperCube(hyperCube));
			...
			for (int j = 0; j < clause.atoms.get(i).terms.size(); j++)
			{
				...
				if(termPosition==-1)
				{
					...
					for(int k=0;k<clause.atoms.get(i).terms.get(j).domain.size();k++)
						term.domain.add(new TreeSet<Integer>(clause.atoms.get(i).terms.get(j).domain.get(k)));
					term.origDomain.addAll(clause.atoms.get(i).terms.get(j).origDomain);
					newTerms.add(term);
	...
	public static Atom create_new_atom(Atom atom) {
		...
		newAtom.activeTermsIndices = new ArrayList<Integer>(atom.activeTermsIndices);
		// copy hypercubes
	...
	public static Term create_new_term(Term term) {
		int type = term.type;
		List<Set<Integer>> domain = new ArrayList<Set<Integer>>();
		for(int k=0;k<term.domain.size();k++){
			Set<Integer> s = new TreeSet<Integer>(term.domain.get(k));
			domain.add(s);
		}
		List<Integer> origDomain = new ArrayList<Integer>(term.origDomain);
		return new Term(type,domain, origDomain);
	}
	...
	*/
	...
	public void setMLNPoperties()
	{
		...
		for(WClause clause : clauses){
			for(Atom atom : clause.atoms){
				if(!symbols.contains(atom.symbol)){
					symbols.add(atom.symbol);
				}
			}
		}
		//max degree of MLN
	...
	public boolean isPropositionalSegments(){
		for(WClause clause : clauses){
			for(Atom atom : clause.atoms){
				for(Term term : atom.terms){
					Set<Integer> constants = new HashSet<Integer>();
					for(Set<Integer> segment : term.domain){
						constants.addAll(segment);
					}
					if(constants.size() > 1)
						return false;
				}
	...
	public boolean isPropositionalActiveTerms(){
		for(WClause clause : clauses){
			if(clause.isPropositionalActiveTerms() == false){
				return false;
			}
		}
		return true;
