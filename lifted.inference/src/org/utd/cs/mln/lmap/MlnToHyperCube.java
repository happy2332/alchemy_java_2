...
public class MlnToHyperCube {
	...
	public void createPredsTupleHashMap(MLN mln, ArrayList<Evidence> evidList, HashMap<PredicateSymbol,ArrayList<ArrayList<TupleConstants>>> predTuplesHashMap, boolean queryEvidence, ArrayList<String> queries){
		for(Evidence evid : evidList){
			...
			if(queryEvidence == true){
				if(queries.contains(predSymbol.symbol)){
					predTuplesHashMap.get(predSymbol).get(2).add(new TupleConstants(constants));
				}
				else{
					if(truthVal == true){
		...
		// Add those symbols which didn't appear in evidence file
	...
	public HashMap<PredicateSymbol,ArrayList<ArrayList<HyperCube>>> createPredsHyperCube(ArrayList<Evidence> evidList, MLN mln, boolean isNormal, ArrayList<String> queries, boolean queryEvidence){
		...
		createPredsTupleHashMap(mln, evidList, predsTuplesHashMap, queryEvidence, queries);
		/*//
		...
		for(PredicateSymbol predSymbol : predsTuplesHashMap.keySet()){
			...
			for(int i = 0 ; i < 2 ; i++){
				...
				ArrayList<HyperCube> mergedHyperCubes = hyperCubes;
				if(isNormal == false){
					mergedHyperCubes = chcb.mergeHyperCubes(hyperCubes);
				}
			...
			ArrayList<HyperCube>remainingHyperCubes = new ArrayList<HyperCube>();
			...
			if(isNormal == false){
				// Merge true and false hypercubes so that we get efficient remaining hypercube
			...
			remainingHyperCubes = chcb.createComplementaryHyperCubes(bothTrueFalseHyperCubes, predDomainHyperCube);
			// If pred is a query
			if(queries.contains(predSymbol.symbol)){
				// query is open world
				if(LiftedPTP.queryEvidence == false){
					predsHyperCubesHashMap.get(predSymbol).set(2, remainingHyperCubes);
				}
				// query is closed world, so add into false hyperCubes
				else{
					predsHyperCubesHashMap.get(predSymbol).get(1).addAll(remainingHyperCubes);
					if(isNormal == false){
						predsHyperCubesHashMap.get(predSymbol).set(1, chcb.mergeHyperCubes(predsHyperCubesHashMap.get(predSymbol).get(1)));
					}
				}
			}
			// If pred is evidence, its always closed world
			else{
				predsHyperCubesHashMap.get(predSymbol).get(1).addAll(remainingHyperCubes);
				if(isNormal == false){
					predsHyperCubesHashMap.get(predSymbol).set(1, chcb.mergeHyperCubes(predsHyperCubesHashMap.get(predSymbol).get(1)));
	...
	public ArrayList<WClause> createClauseHyperCube(WClause clause, HashMap<PredicateSymbol,ArrayList<ArrayList<HyperCube>>> predsHyperCubeHashMap, boolean isNormal){
		...
		for(int clauseId = 0 ; clauseId < numClauses ; clauseId++){
			...
			if(clauseExists == false  || allFalseAtoms == true){
				continue;
			}
			...
			if(numAtoms == 1){
				finalAtom.terms = new ArrayList<Term>(atom1.terms);
			...
			for(int atomId = 1 ; atomId < numAtoms ; atomId++){
				...
				atom1.hyperCubes.clear();
				atom.hyperCubes.clear();
				atom1 = finalAtom;
				if(finalAtom.hyperCubes.size() == 0){
					clauseExists = false;
					break;
				}
			...
			// Remove false and true atoms from this clause
			for(int atomId = numAtoms - 1 ; atomId >= 0 ; atomId--){
				if(atomSigns.get(atomId) == 1 || atomSigns.get(atomId) == 0){
					newClause.atoms.remove(atomId);
					newClause.sign.remove(atomId);
				}
			}
			
			for(Term t : finalAtom.terms){
				t.domain.clear();
			}
			int numHyperCubes = finalAtom.hyperCubes.size();
			newClause.hcNumCopies.clear();
			newClause.satHyperCubes.clear();
			for(int i = 0 ; i < numHyperCubes ; i++){
				newClause.hcNumCopies.add(1);
				if(newClause.satisfied == true)
					newClause.satHyperCubes.add(true);
				else
					newClause.satHyperCubes.add(false);
				for(Term t : finalAtom.terms){
					int termId = finalAtom.terms.indexOf(t);
					t.domain.add(new TreeSet<Integer>(finalAtom.hyperCubes.get(i).varConstants.get(termId)));
					if(!termsToStay.contains(t)){
						int numCopies = newClause.hcNumCopies.get(i);
						newClause.hcNumCopies.set(i,numCopies*finalAtom.hyperCubes.get(i).varConstants.get(termId).size());
					}
				}
			}
			
			if(newClause.atoms.size() == 0){
				for(int i = 0 ; i < numHyperCubes ; i++){
					if(newClause.satHyperCubes.get(i) == true){
						totalSatisfiedWeight += newClause.weight.value*newClause.hcNumCopies.get(i);
					}
				}
			}
			else{
		...
		if(totalSatisfiedWeight > 0){
			WClause emptyClause = MLN.create_new_clause(clause);
			emptyClause.atoms.clear();
			emptyClause.sign.clear();
			emptyClause.satHyperCubes.clear();
			emptyClause.hcNumCopies.clear();
			emptyClause.satHyperCubes.add(true);
			emptyClause.hcNumCopies.add(1);
			emptyClause.weight.value = totalSatisfiedWeight;
			resultClauses.add(emptyClause);
		}
		...
		for(int atom1HcId = 0 ; atom1HcId < atom1.hyperCubes.size() ; atom1HcId++){
			for(int atom2HcId = 0 ; atom2HcId < atom2.hyperCubes.size() ; atom2HcId++){
				...
				if(!isEmptyHyperCube){
					for(int termId = 0 ; termId < atom2.terms.size() ; termId++){
						if(!atom2CommonTermIndices.contains(termId)){
							finalHyperCube.varConstants.add(new TreeSet<Integer>(atom2.hyperCubes.get(atom2HcId).varConstants.get(termId)));
						}
					}
