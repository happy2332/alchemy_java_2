...
public class Decomposer {
	...
	public static void createDisjointSegments(int h1Id, int h2Id, ArrayList<ArrayList<Integer>> hyperCubesIntersectionList, ArrayList<HyperCube>hyperCubes){
		HyperCube h1 = hyperCubes.get(h1Id);
		HyperCube h2 = hyperCubes.get(h2Id);
		//System.out.println("Inside createDisjointHyperCubesVar function with input hyperCubes : "+h1.toString()+" and "+h2.toString());
		// create 3 sets : intSectHc = h1^h2, h1-intSectHc, h2-intSectHc, all on variable varId
	...
	public static void createDisjointHyperCubes(ArrayList<HyperCube>segments){
		if(segments.size() < 2){
			return;
		}
		...
		for(int hc1Id = 0 ; hc1Id < segments.size() ; hc1Id++){
			for(int hc2Id = 0 ; hc2Id < hc1Id ; hc2Id++){
				...
				*//*
				...
				if(!segments.get(hc1Id).areDisjointOrIdenticalVar(segments.get(hc2Id), 0)){
					hyperCubesIntersectionList.get(hc1Id).add(hc2Id);
					hyperCubesIntersectionList.get(hc2Id).add(hc1Id);
					/*
					if(varId == 1 && hc1Id == 1 && hc2Id == 0){
		...
		while(true){
			...
			//System.out.println("Now calling disjoint on var function with hcListId = "+hcListId+" and hc2Id = "+hc2Id);
			//System.out.println(hyperCubes);
			createDisjointSegments(hcListId,hc2Id,hyperCubesIntersectionList,segments);
			//System.out.println(hyperCubes);
			//System.out.println(hyperCubesIntersectionList);
		}
		//System.out.println("Disjoint on variable "+ varId + "done, now number of hyperCubes = "+hyperCubes.size());
		// Remove duplicates
		Set<HyperCube> segmentsSet = new HashSet<HyperCube>(segments);
		segments.clear();
	...
	public ArrayList<MLN> initDecomposer(MLN mln, Set<Pair> equiClass, ArrayList<Integer> sizeOfSegments){
	...
	private void createDecomposedMLNs(
		...
		for(HyperCube segment : hyperCubeClauseListHashMap.keySet()){
			...
			for(Integer clauseId : hyperCubeClauseListHashMap.get(segment)){
				...
				decomposerClause.hyperCubes.clear();
				...
				for(int hcId = parentClause.hyperCubes.size() - 1 ; hcId >= 0 ; hcId--){
					...
					if(newHyperCube.varConstants.get(equiClassTermIndex).contains(segmentFirstElement)){
						...
						decomposerClause.hyperCubes.add(newHyperCube);
					}
