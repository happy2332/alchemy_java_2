...
public class LiftedSplit {
	...
	public static double liftedSplit(MLN mln, boolean approximate){
		...
		//System.out.println("No. of preds = " + predIdMap.size());
		//System.out.println("predIdMap = " + predIdMap);
		/*
		if(count++%100000==0)
	...
	private static double binomialSplitter(MLN mln, int predId,
		...
		if(approximate == true){
			...
			LiftedPTP.totalSegments += segment.varConstants.get(0).size();
			LiftedPTP.logComb += Math.log(comb.findComb(segmentSize, caseSelected));
			...
			for(Integer termIndex : clausesBinomialTermIndices.get(clauseId)){
				...
				for(int hcId = 0 ; hcId < origNumHyperCubes ; hcId++){
					if(clause.hyperCubes.get(hcId).varConstants.get(termIndex).contains(segmentFirstElement)){
						...
						clause.hyperCubes.get(hcId).varConstants.set(termIndex, new TreeSet<Integer>(segmentSet)); // A
					}
				}
		...
		for(int k = choice ; k <= endIndex ; k++){
			...
			for(int clauseId = 0 ; clauseId < parentMln.clauses.size() ; clauseId++){
				...
				for(int hcId = newClause.hyperCubes.size() - 1 ; hcId >= 0 ; hcId--){
					...
					ArrayList<ArrayList<TreeSet<Integer>>> sameSegmentsPartitions = new ArrayList<ArrayList<TreeSet<Integer>>>();
					for(int i = 0 ; i < sameSegmentOccurs.size() ; i++){
						if(sameSegmentOccurs.get(i) == true){
							TreeSet<Integer> falsePartiton = new TreeSet<Integer>(segment.varConstants.get(0));
							TreeSet<Integer> truePartiton = new TreeSet<Integer>(segment.varConstants.get(0));
	...
	public static ArrayList<ArrayList<TreeSet<Integer>>> cartesianProdVar(ArrayList<TreeSet<Integer>> sets, TreeSet<Integer> toGroundTermIdsSet){
		ArrayList<ArrayList<TreeSet<Integer>>> result = new ArrayList<ArrayList<TreeSet<Integer>>>();
		int numTuples = 1;
		...
		for(int i = 0 ; i < numTuples ; i++){
			result.add(new ArrayList<TreeSet<Integer>>());
		}
		...
		for(int i = 0 ; i < sets.size() ; i++){
			...
			if(!toGroundTermIdsSet.contains(i)){
				for(int j = 0 ; j < numTuples ; j++){
					result.get(j).add(new TreeSet<Integer>(sets.get(i)));
				}
			}
				...
				while(numFilledEntries != numTuples){
					for(Integer elem : sets.get(i)){
						for(int j = 0 ; j < numRepeats ; j++){
							result.get(numFilledEntries).add(new TreeSet<Integer>(Arrays.asList(elem)));
							numFilledEntries++;
						}
					}
				}
	...
	public static ArrayList<ArrayList<TreeSet<Integer>>> cartesianProd(ArrayList<TreeSet<Integer>> sets){
		ArrayList<ArrayList<TreeSet<Integer>>> result = new ArrayList<ArrayList<TreeSet<Integer>>>();
		int numTuples = 1;
		for(int i = 0 ; i < sets.size() ; i++){
			numTuples *= sets.get(i).size();
	...
	public static void createGroundMlnPred(MLN mln, int predId){
		for(WClause clause : mln.clauses){
			...
			TreeSet<Integer> toGroundTermIdsSet = new TreeSet<Integer>();
			for(Atom atom : clause.atoms){
				if(atom.symbol.id == predId){
					for(Term term : atom.terms){
			...
			if(toGroundTermIdsSet.size() > 0){
				...
				for(int hcId = origNumHyperCubes - 1 ; hcId >= 0 ; hcId--){
					...
					ArrayList<ArrayList<TreeSet<Integer>>> groundedHyperCubes = cartesianProdVar(hc.varConstants, toGroundTermIdsSet);
					for(ArrayList<TreeSet<Integer>> groundedVarConstants : groundedHyperCubes){
	...
	public static void main(String[] args) throws FileNotFoundException, PredicateNotFound {
		...
		mln.clauses.get(0).hyperCubes.get(0).varConstants.set(0,new TreeSet<Integer>(Arrays.asList(0,1)));
		mln.clauses.get(0).hyperCubes.get(0).varConstants.set(1,new TreeSet<Integer>(Arrays.asList(1,2)));
